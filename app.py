...
def render_full_analysis(df, title_prefix, selected_months_list, df_streak=None):
    ...
    with col_payout:
        st.markdown(f"""
            <div style="height: 260px; padding: 20px; border-radius: 12px; background-color: #1a1c24; border-left: 5px solid #00cf8d; display: flex; flex-direction: column; justify-content: center; align-items: center; box-sizing: border-box; gap: 10px;">
                <div><p style="margin:0; opacity:0.7; font-size:14px; text-transform: uppercase; text-align: center;">Total Payout ({payout_days} zile)</p>
                <h1 style="margin:5px 0; color:#00cf8d; font-size:42px; line-height: 1; text-align: center;">${total_pay:,.2f}</h1></div>
                ...
                </div><p style="margin:0; font-size:13px; opacity:0.5;">{len(cycles) if cycles else 0} cicluri identificate</p>
    ...
    if cycles:
        ...
        fig_p = px.bar(monthly_pay, x='Luna_Formatata', y='Payout', title=f"Profit Net pe Luni (Ciclu {payout_days} zile)", text_auto='.2s', template="plotly_dark", color_discrete_sequence=['#00cf8d'])
        ...
        with st.expander("Vezi Timeline Detaliat"):
    ...
    def style_streak_row(row):
        if row['Șir curent'] == active_label:
            color = '#0d2111' if 'Win' in active_label else '#210d0d'
            return [f'background-color: {color}; color: white; border-color: #ffffff ; font-weight: bold'] * len(row)
        return [''] * len(row)

    col_p1, col_p2 = st.columns(2)
    with col_p1: 
        if not df_win_prob.empty: st.table(df_win_prob.style.apply(style_streak_row, axis=1))
        else: st.write("Lipsă date Win streaks")
    with col_p2: 
        if not df_loss_prob.empty: st.table(df_loss_prob.style.apply(style_streak_row, axis=1))
        else: st.write("Lipsă date Loss streaks")

    st.markdown("---")
    st.subheader("⏰ Analiză pe Ore (INTRARE)")
    hour_stats = df.groupby('Hour').agg(Profit=('Net P&L USD', 'sum'), W=('Result', lambda x: (x == 'Win').sum()), L=('Result', lambda x: (x == 'Loss').sum()), Total_Trades=('Result', 'count'), WR=('Result', lambda x: (len(x[x=='Win'])/len(x))*100 if len(x)>0 else 0)).reset_index()
    hour_stats['Time Label'] = hour_stats['Hour'].apply(lambda x: f"{int(x):02d}:00")
    fig_hour = px.bar(hour_stats, x='Time Label', y='Profit', text=hour_stats.apply(lambda r: f"${r['Profit']:,.0f}<br>{r['WR']:.0f}% ({int(r['W'])}W/{int(r['L'])}L)", axis=1), template="plotly_dark", color='Profit', color_continuous_scale='RdYlGn')
    fig_hour.update_traces(textposition='outside')
    st.plotly_chart(fig_hour, use_container_width=True, key=f"hour_entry_{title_prefix}")

    top_col, bottom_col = st.columns(2)
    with top_col:
        st.markdown("#### 🟢 Top 5 Winning Hours")
    ...
    with bottom_col:
        st.markdown("#### 🔴 Top 5 Losing Hours")
    ...
    exit_hour_stats = df.dropna(subset=['Exit_Hour']).groupby('Exit_Hour').agg(Profit=('Net P&L USD', 'sum'), W=('Result', lambda x: (x == 'Win').sum()), L=('Result', lambda x: (x == 'Loss').sum()), Total_Trades=('Result', 'count'), WR=('Result', lambda x: (len(x[x=='Win'])/len(x))*100 if len(x)>0 else 0)).reset_index()
    exit_hour_stats['Time Label'] = exit_hour_stats['Exit_Hour'].apply(lambda x: f"{int(x):02d}:00")
    fig_exit_hour = px.bar(exit_hour_stats, x='Time Label', y='Profit', text=exit_hour_stats.apply(lambda r: f"${r['Profit']:,.0f}<br>{r['WR']:.0f}% ({int(r['W'])}W/{int(r['L'])}L)", axis=1), template="plotly_dark", color='Profit', color_continuous_scale='RdYlGn')
    fig_exit_hour.update_traces(textposition='outside')
    st.plotly_chart(fig_exit_hour, use_container_width=True, key=f"hour_exit_{title_prefix}")

    st.markdown("---")
    st.subheader("📊 Performanță pe Zile")
    day_stats = df.groupby('Day').agg(Profit=('Net P&L USD', 'sum'), W=('Result', lambda x: (x == 'Win').sum()), L=('Result', lambda x: (x == 'Loss').sum()), WR=('Result', lambda x: (len(x[x=='Win'])/len(x))*100 if len(x)>0 else 0)).reindex(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday']).dropna(subset=['Profit']).reset_index()
    fig_day = px.bar(day_stats, x='Day', y='Profit', text=day_stats.apply(lambda r: f"${r['Profit']:,.0f}<br>{r['WR']:.0f}% ({int(r['W'])}W/{int(r['L'])}L)", axis=1), template="plotly_dark", color='Profit', color_continuous_scale='RdYlGn')
    ...
    month_stats = df.groupby('Month').agg(Profit=('Net P&L USD', 'sum'), W=('Result', lambda x: (x == 'Win').sum()), L=('Result', lambda x: (x == 'Loss').sum()), WR=('Result', lambda x: (len(x[x=='Win'])/len(x))*100 if len(x)>0 else 0)).reset_index()
    month_stats['Month'] = pd.Categorical(month_stats['Month'], categories=['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'], ordered=True)
    month_stats = month_stats.sort_values('Month').dropna(subset=['Profit'])
    fig_month = px.bar(month_stats, x='Month', y='Profit', text=month_stats.apply(lambda r: f"${r['Profit']:,.0f}<br>{r['WR']:.0f}% ({int(r['W'])}W/{int(r['L'])}L)", axis=1), template="plotly_dark", color='Profit', color_continuous_scale='RdYlGn')
    fig_month.update_traces(textposition='outside')
    st.plotly_chart(fig_month, use_container_width=True, key=f"month_bar_{title_prefix}")

    st.markdown("---")
    st.subheader("📅 Profit pe Ani")
    yearly = df.groupby('Year').agg(Profit=('Net P&L USD', 'sum'), W=('Result', lambda x: (x == 'Win').sum()), L=('Result', lambda x: (x == 'Loss').sum()), WR=('Result', lambda x: (len(x[x=='Win'])/len(x))*100 if len(x)>0 else 0)).reset_index()
    fig_yr = px.bar(yearly, x='Year', y='Profit', text=yearly.apply(lambda r: f"${r['Profit']:,.0f}<br>{r['WR']:.0f}% ({int(r['W'])}W/{int(r['L'])}L)", axis=1), template="plotly_dark", color='Profit', color_continuous_scale='Greens')
    ...
    st.plotly_chart(px.line(df_sorted, x='Entry Time', y='Cumulative', template="plotly_dark", color_discrete_sequence=['#00cf8d']), use_container_width=True, key=f"equity_line_{title_prefix}")

# 4. LOGICĂ DATE + FILTRE
col_left, col_mid, col_right = st.columns([4, 1, 4])
with col_mid:
    st.image("logo-lvlup.png", width=200) 
st.markdown("<h1 style='text-align: center;'>TradingView Payout & Strategy Tester</h1>", unsafe_allow_html=True)
...
if uploaded_file:
    try:
        ...
        df_combined['Direction'] = df_combined['Type_y'].apply(lambda x: 'Long' if 'long' in str(x).lower() else ('Short' if 'short' in str(x).lower() else 'Other'))
        ...
        df_combined['Session'] = df_combined['Entry Time'].apply(lambda x: "Sesiunea 1" if x.time() < datetime.strptime("15:30", "%H:%M").time() else "Sesiunea 2")

        st.markdown("### 🔍 Filtrare Date")
        c1, c2, c3, c4 = st.columns(4)
        with c1: selected_years = st.multiselect("Anii:", sorted(df_combined['Year'].unique()), default=sorted(df_combined['Year'].unique()))
        with c2: selected_months = st.multiselect("Lunile:", ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'], default=[m for m in ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'] if m in df_combined['Month'].unique()])
